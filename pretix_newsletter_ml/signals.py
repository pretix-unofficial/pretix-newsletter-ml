...
def order_placed(sender, order, **kwargs):
    skip = not sender.settings.newsletter_ml_subscribe_address or (
        not sender.settings.newsletter_ml_add_automatically
        and not order.meta_info_data.get("contact_form_data", {}).get("ml_newsletter")
        is True
    )
    if skip:
        return
...
def cf_formfields(sender, **kwargs):
    skip = (
        not sender.settings.newsletter_ml_subscribe_address
...
def pretixcontrol_logentry_display(sender, logentry, **kwargs):
    ...
    plains = {
        "pretix_newsletter_ml.subscribe": _(
            "A subscribe request for the mailing list has been sent."
        ),
    }
